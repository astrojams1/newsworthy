...
const regressions = [
  ...
  ['stale iOS background identity', s => { s.swift = s.swift.replace('.id("\\(entry.reading?.score ?? 0)-\\(entry.appearance.rawValue)")', '.id(1)'); }, /background change identity/],
  ['iOS background identity ignores widget appearance', s => { s.swift = s.swift.replace('.id("\\(entry.reading?.score ?? 0)-\\(entry.appearance.rawValue)")', '.id(entry.reading?.score)'); }, /background change identity/],
  ['wrong iOS level', s => { s.swift = s.swift.replace('WidgetSurface(score: entry.reading?.score, scheme:', 'WidgetSurface(score: 1, scheme:'); }, /background change identity/],
  ['iOS widget background ignores chosen appearance', s => { s.swift = s.swift.replace('.modifier(ForcedColorScheme(scheme: scheme))', ''); }, /extracted background follows the chosen appearance/],
  ['iOS widget text ignores chosen appearance', s => { s.swift = s.swift.replace('.modifier(ForcedColorScheme(scheme: entry.appearance.colorScheme))', ''); }, /content follows the chosen appearance/],
  ['Android Follow device bakes colours', s => { s.java = s.java.replace('if (dark != null) applyChosenAppearance(', 'applyChosenAppearance('); }, /only a chosen widget appearance/],
  ...
  ['iOS widget appearance missing from widget settings', s => { s.swift = s.swift.replace('@Parameter(title: "Appearance")', ''); }, /appearance is a widget setting/],
  ['iOS widget appearance not applied to timeline', s => { s.swift = s.swift.replace('                    entry.appearance = configuration.appearance?.value ?? .system\n', ''); }, /settings reach snapshot and timeline/],
