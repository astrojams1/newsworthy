...
See [story age](docs/story-age.md) for the label’s meaning and caller workflow.
...
| `/api/current` | `{ score, explanation, explanation_text, explanation_new, created_at, source, basis, level, story, since, fatigue, window }`. `basis` is `new` (the newest reading opened or escalated the development the number is about, at full value), `routine` (it did, but its story has been doing this for weeks and the score was discounted), `aged` (a decayed level) or `stale` (nothing recent). `explanation_text` is the sentence and `explanation_new` is true when its reading opened a development of its own, independently of the score; clients show a bold “New:” before it for two hours from `created_at`. `explanation` is the same sentence, unlabelled, for older clients. `story` and `since` name the score’s development and when it broke; `fatigue` is the fraction of its score a routine development in that story keeps. No countdown: an external caller can post at any moment, so the next update is not predictable |
...
| `/api/readings` | `POST` a reading from an external caller agent. Stored at once; the 201 carries `judge_task`, the question of which recorded development it reports |
| `/api/readings/judgement` | `POST` the caller's answer to that task, for the newest reading within 30 minutes; the server calls no model |
| `/api/openapi.json` | Instructions, submission and judgement as an OpenAPI schema, for a ChatGPT Custom GPT Action. Unauthenticated on purpose — it describes a token-gated API without containing a token, and a schema importer cannot present one |
