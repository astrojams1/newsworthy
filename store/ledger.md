...
Updated: 2026-09-26T09:05:43+00:00
...
| widget.appearance-choice | waiting_verification | agent | observed | Defect from today's simulator run fixed in source, not yet verified: iOS 26.5 passed the Appearance AppEnum parameter to the widget as nil, so Light and Dark followed the system (iOS 18.6 decoded the same binary). Appearance is now an AppEntity (AppearanceOption, three fixed options, Follow device as the query default and when unset); the intent name and Show app name are unchanged so existing widgets keep their configuration. The entity code has not been compiled or run: the EAS account's free-plan build credits and 60 CI/CD sandbox minutes are both used up until 1 Oct 2026 UTC, and this container has no Swift toolchain or KVM. Observed earlier today on the #152 code: iOS 18.6 both sizes pass checks 1 and 2 and new widgets default to Follow device with the name shown; iOS 26.5 passes check 1 and fails check 2. | After 1 Oct (or a plan upgrade): compile the widget, then on iOS 26.5 and 18.6 simulators, both sizes: Appearance Light under system Dark, Dark under Light, Follow device switching, Show app name off/on; upgrade from build 21 (existing widgets default to Follow device with the name shown); iOS 16.4 shows the widget with no settings; save screenshots under store/source/. Then a TestFlight build on a real iOS 26 device. Android: every check on a device or KVM emulator (preview APK), including the settings-screen look, Android 11 close-without-Done and TalkBack. |
...
### 214. apple.testflight-15 — in_progress

2026-09-22T11:50:51+00:00 · observed · agent

Owner requested updated TestFlight. Packaging clean origin/main 65cafc3 with merged widget alignment and story-age changes as next iOS production build. Existing latest TestFlight build is 14.

- User request 2026-09-22; EAS latest-build readback; source origin/main 65cafc3.

Next: Run release checks, build signed iOS package, upload exact build and verify existing Release QA availability; record native-device verification limits.

### 215. apple.testflight-15 — waiting_provider

2026-09-22T11:53:26+00:00 · observed · provider

EAS is compiling iOS 1.0.0 build15 from origin/main 65cafc3. Application source is clean; only ledger edits were present during archive preparation. Tests passed256, skipped1; design, TypeScript and mobile configuration checks passed.

- EAS build2f8440c5-571b-4aa7-bf6d-3569e0fff3b6; production profile; source65cafc3cd39855977c9ff4d03329a10114c266a8.

Next: Verify finished signed package, upload exact build15 and read back Apple processing and Release QA group status.

### 216. apple.testflight-15-build — done

2026-09-22T11:59:02+00:00 · observed · agent

EAS finished signed iOS1.0.0 build15 at2026-09-22T11:57:52Z from merged main65cafc3.

- EAS2f8440c5-571b-4aa7-bf6d-3569e0fff3b6 FINISHED; exact source65cafc3cd39855977c9ff4d03329a10114c266a8; production profile.

### 217. apple.testflight-15-upload — in_progress

2026-09-22T12:00:40+00:00 · observed · agent

Downloaded IPA passes ZIP integrity; main app and widget both1.0.0(15), expected bundle identifiers, matching signed App Groups and embedded provisioning entitlements. Submitting exact EAS build15.

- store/testflight-15.json; SHA256809403621664382a6d30192afcde7451b71f5e4c0a18d8e44d06f1c461acc1b8.

Next: Read submission receipt, Apple processing state and Release QA availability.

### 218. apple.testflight-15-upload — waiting_provider

2026-09-22T12:01:20+00:00 · observed · provider

EAS accepted upload submission of exact iOS1.0.0 build15 using the existing Apple key.
...
2026-09-23T23:21:59+00:00 · observed · agent

PR127 merged as3a2f0e0; both GitHub checks passed. Vercel production alias now serves deploymentdpl_2d1c1kXXV4ej2Vp1r2znLRVW4DAT with receipt/retry fixes and updated privacy text. Home, Settings, current API, health and privacy return200.

- https://github.com/astrojams1/newsworthy/pull/127; Vercel Ready readback; production HTTP smoke checks September24

### 254. apple.build — done

2026-09-23T23:29:06+00:00 · observed · agent

Production iOS build 21 finished from 2d7f56d, matching merged PR127 application source. App and widget versions, production push entitlement, provisioning profiles, App Groups, privacy manifest, ZIP integrity and code signatures verified. Build 20 is canceled.

- store/testflight-21.json

### 255. apple.review — in_progress

2026-09-23T23:29:07+00:00 · observed · agent

App Review correspondence read back: the owner reconsideration was received, and Apple upheld Guideline 4.2 on September 23. Version remains REJECTED; build 7 is still selected. User authorized finishing notifications and resubmitting a changed build.

- September 24 signed-in App Store Connect correspondence; store/testflight-21.json

Next: Complete push-key configuration and physical delivery/tap verification, save revised review notes and listing, select valid build 21, then resubmit and verify Waiting for Review.

### 256. apple.notifications — waiting_user

2026-09-23T23:31:06+00:00 · observed · user

Notification code merged and deployed; 292 tests and 70 design checks passed. One iOS device is registered at threshold 5. No APNs delivery key exists in Apple or Expo; the production Newsworthy-specific key is prepared and awaits action-time security confirmation. Physical receipt and tap verification remain pending.

- store/testflight-21.json; store/user-actions.md

Next: Owner confirms the pending push-key question; agent creates/configures the key, sends a labeled test, and owner confirms physical receipt and opening on build 21.

### 257. apple.upload — waiting_provider

2026-09-23T23:32:46+00:00 · observed · provider

Apple validation and direct upload succeeded for build 21. Delivery UUID 6ce55c01-2cbf-4d35-b29f-b010df9502cb. Awaiting API build processing; selected App Review build remains 7.
...
### 303. widget.appearance-choice — failed

2026-09-26T09:02:11+00:00 · observed · agent

iOS simulators, shipped code (fae4cc6, Release, simulator build in an EAS macOS sandbox; Swift compiled cleanly, x86_64 and arm64). iOS 18.6 (iPhone 16), both sizes: Edit Widget offers Show app name (on) and Appearance (Follow device) on a new widget; name off removes the heading and moves rating and sentence up about 11 pt at the same 69-pt numeral, name on restores it; Light held with the system in Dark, Dark held with the system in Light, Follow device switched with the system; settings are per widget and persist. iOS 26.5 (iPhone 17): Show app name works the same, but Appearance has no effect: Light under a Dark system and Dark under a Light system both follow the system. The widget log shows the saved choice arriving (serializedParameters appearance = dark) and then 'Programming error: a caseDisplayRepresentations entry for AppEnum case "to-0.0" was not found' and 'Prepared appearance to WidgetAppearance(nil)', so the enum decodes to nil and falls back to Follow device. The same binary decodes it on iOS 18.6. Not changed by -Onone, static var representations, explicit allCases, or renaming the type or cases (renaming dropped an existing widget from the home screen). Judged from background luminance and Vision text recognition of simulator screenshots; the sandbox stopped before images could be copied out.

- Simulator measurements: iOS 18.6 medium Follow device 227 light / 51 dark system; small Dark 41 under Light; medium Light 227 under Dark; iOS 26.5 medium Light 59 under Dark, small Dark 237 under Light (luminance 0-255)

Next: Find an Appearance parameter that decodes on iOS 26 (and check whether real iOS 26 devices are affected, via TestFlight once EAS build credits reset on 1 Oct). Untested: upgrade from build 21 (check 3), iOS 16 (check 5), every Android check (no KVM here, no hypervisor in the EAS sandbox, EAS cloud builds blocked by exhausted free-plan credits).

### 304. widget.appearance-choice — waiting_verification

2026-09-26T09:05:43+00:00 · observed · agent

Defect from today's simulator run fixed in source, not yet verified: iOS 26.5 passed the Appearance AppEnum parameter to the widget as nil, so Light and Dark followed the system (iOS 18.6 decoded the same binary). Appearance is now an AppEntity (AppearanceOption, three fixed options, Follow device as the query default and when unset); the intent name and Show app name are unchanged so existing widgets keep their configuration. The entity code has not been compiled or run: the EAS account's free-plan build credits and 60 CI/CD sandbox minutes are both used up until 1 Oct 2026 UTC, and this container has no Swift toolchain or KVM. Observed earlier today on the #152 code: iOS 18.6 both sizes pass checks 1 and 2 and new widgets default to Follow device with the name shown; iOS 26.5 passes check 1 and fails check 2.

- test/widget-settings.test.js rejects an AppEnum choice, a drifted default and a drifted fallback; design gate 98 passed; simulator luminance and Vision text-recognition measurements are in the previous event

Next: After 1 Oct (or a plan upgrade): compile the widget, then on iOS 26.5 and 18.6 simulators, both sizes: Appearance Light under system Dark, Dark under Light, Follow device switching, Show app name off/on; upgrade from build 21 (existing widgets default to Follow device with the name shown); iOS 16.4 shows the widget with no settings; save screenshots under store/source/. Then a TestFlight build on a real iOS 26 device. Android: every check on a device or KVM emulator (preview APK), including the settings-screen look, Android 11 close-without-Done and TalkBack.
