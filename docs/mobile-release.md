...
- iOS: `targets/widget` contains a SwiftUI/WidgetKit extension configured by
  ...
  and saved per widget. Turning it off hides the heading inside either widget
  size. iOS 16 keeps the heading visible. The footer uses a compact, single-line
  date and time, retaining the saved-reading label when offline.
