...
4. Submit score, explanation and prompt_sha256 to `/api/readings`. Confirm the API
   stored it. The reading is committed before you see any history.
5. Answer the `judge_task` in the 201 at `/api/readings/judgement`: `reading`,
   `judge_version`, `development_of` (an id the task lists, or null for a new
   development), `story` (reuse a listed name verbatim when the story is on
   record) and `note`. Newsworthy calls no model for this. If the reply says
   `"judged": false`, correct the answer per its `reason` and send it again.

The service supplies the recent history inside the task, after the reading is stored; the caller does not need
